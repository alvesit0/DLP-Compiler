...
public class IdentificationTest {
  ...
  void duplicatedRecordField() {
    ...
    assertFoundErrors(Arrays.asList(new Error(4, 5, ErrorReason.FIELD_ALREADY_DECLARED)));
  }
  ...
  void duplicatedFunctionDefinition() {
    ...
    assertFoundErrors(Arrays.asList(new Error(4, 1, ErrorReason.FUNCTION_ALREADY_DECLARED)));
  }

  @Test
  void missingVarDefinition() {
    runCompiler("examples/errors/identification/missing_var_declaration.xana");
    assertFoundErrors(Arrays.asList(new Error(4, 5, ErrorReason.VARIABLE_NOT_DECLARED)));
  }

  @Test
  void missingFunctionDefinition() {
    runCompiler("examples/errors/identification/missing_function_declaration.xana");
    assertFoundErrors(Arrays.asList(new Error(5, 5, ErrorReason.FUNCTION_NOT_DECLARED)));
  }
